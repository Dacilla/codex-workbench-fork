...
- [ ] Synthetic MCP server receives large request byte-for-byte unchanged — OPEN (transport level). Integrity asserted at TUI boundary only (args serialization-identical pre/post render + replay round-trip). Needs core-side synthetic-server test.
